...
public class SearchAHTask implements Runnable {
    ...
    private String requestAHPage(String token) throws UnsupportedEncodingException {
        ...
        if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
            ...
            return URLDecoder.decode(obj.getJSONObject(url).getJSONObject("headers").getJSONArray("location").get(0).toString(), "UTF-8");
        } else {
            log.error("Error while requesting AH page. Code: {}, Body: {}.", response.getStatusCode(), response.getBody());
        }
        return null;
    }
    ...
    private String requestAHSearch(String search, String token) {
        ...
        if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
            ...
            log.error("Error while requesting AH search. Code: {}, Body: {}.", response.getStatusCode(), response.getBody());
        }
        return null;
    }

    private void requestBuy(String buyData, String token) {
        ResponseEntity<String> response = httpClient.makePOSTRequest("http://210.140.157.168/card/trade_buy.htm", "POST", buyData, token);
        if (response.getStatusCode() != HttpStatus.OK || response.getBody() == null) {
            log.error("Error while requesting Token. Code: {}, Body: {}.", response.getStatusCode(), response.getBody());
        }
    }
    ...
    private void updateToken() {
        ...
        params.put("st", token);
        ResponseEntity<String> response = httpClient.makePOSTRequest("http://yahoo-mbga.jp/game/12004455/update_token", params, cookie);
        if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
            JSONObject obj = new JSONObject(response.getBody());
            this.token = obj.getString("token");
        } else {
            log.error("Error while updating token. Code: {}, Body: {}.", response.getStatusCode(), response.getBody());
        }
    }
